...
6. Verify via `GET /api/health`, `GET /api/monitoring`, MinIO bucket provisioning
...
**Note:** `CSRF_SECRET` falls back to `AUTH_SECRET` if unset (dev convenience only — always set explicitly in production).
...
- In-progress workflow counts for AI analysis, paper generation, export generation, system backup
