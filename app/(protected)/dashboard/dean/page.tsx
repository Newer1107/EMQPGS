...
export default async function DeanDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <h1 className="text-3xl font-semibold">Dean Dashboard</h1>
      <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-5">
        {data.stats.map((stat) => <StatCard key={stat.label} {...stat} />)}
      </div>
      <Card>
        <CardHeader><CardTitle>Readiness Notes</CardTitle></CardHeader>
        <CardContent className="space-y-3">{data.pendingTasks.map((task) => <p key={task} className="rounded-xl border border-slate-200 p-3 text-sm">{task}</p>)}</CardContent>
      </Card>
    </div>
  );
}
