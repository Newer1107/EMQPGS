  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled && body.success && Array.isArray(body.data) && body.data.length > 0) setCurrentVersionId(body.data[0].id);
      } catch { /* ignore */ }
    })();
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <PageHeader title="UAF Analysis Details" description="Deterministic academic quality indices for this question bank." />
      </div>
      <UafAnalysisOverview questionBankId={bankId} />
      <div className="grid gap-6 lg:grid-cols-2">
        <UafVersionHistory questionBankId={bankId} currentVersionId={currentVersionId ?? undefined} onCompare={handleCompare} selectionMode={true} />
        {compareVersionId && currentVersionId && (
          <UafVersionCompare questionBankId={bankId} versionA={{ id: currentVersionId, versionNumber: 0, createdAt: "" }} versionB={{ id: compareVersionId, versionNumber: 0, createdAt: "" }} onClose={() => setCompareVersionId(null)} />
        )}
      </div>
    </div>
  );
