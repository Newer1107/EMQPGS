import { PrismaClient, ResponsibilityType, ScopeType, ExamType, ExamCycleStatus, UserStatus, SubjectStatus, BatchStatus, BatchSemesterStatus, AcademicYearStatus, GroupAssignment } from "@prisma/client";
...
async function main() {
  console.log("Seeding TCET structure — 2 departments, 2 batches, real faculty, responsibility-based assignments...");
  ...
  // ──────────────────────────────────────────────
  // 5. USERS — real TCET faculty (identity only, no role)
  // ──────────────────────────────────────────────
  const FACULTY = [
    { name: "Dr. R.R. Sedamkar", email: "coe@emqpgs.local", homeDept: comp.id },
    { name: "Dr. Sheetal Rathi", email: "dean@emqpgs.local", homeDept: comp.id },
    { name: "Dr. Harshali P. Patil", email: "coordinator.comp@emqpgs.local", homeDept: comp.id },
    { name: "Dr. Megharani Patil", email: "moderator.comp1@emqpgs.local", homeDept: comp.id },
    { name: "Dr. Rekha Sharma", email: "moderator.comp2@emqpgs.local", homeDept: comp.id },
    { name: "Dr. Rashmi Thakur", email: "moderator.comp3@emqpgs.local", homeDept: comp.id },
    { name: "Dr. Preksha Pareek", email: "contributor1.comp@emqpgs.local", homeDept: comp.id },
    { name: "Dr. Vaishali Nirgude", email: "contributor2.comp@emqpgs.local", homeDept: comp.id },
    { name: "Mr. Vikas Singh", email: "contributor3.comp@emqpgs.local", homeDept: comp.id },
    { name: "Mrs. Lydia Suganya", email: "contributor4.comp@emqpgs.local", homeDept: comp.id },
    { name: "Dr. Vinitkumar Dongre", email: "coordinator.extc@emqpgs.local", homeDept: extc.id },
    { name: "Dr. Lochan Jolly", email: "moderator.extc1@emqpgs.local", homeDept: extc.id },
    { name: "Mr. Sanjeev Ghosh", email: "moderator.extc2@emqpgs.local", homeDept: extc.id },
    { name: "Dr. Sujata Kulkarni", email: "contributor1.extc@emqpgs.local", homeDept: extc.id },
    { name: "Dr. Sunita Pachori", email: "coordinator.hns@emqpgs.local", homeDept: hns.id },
    { name: "Dr. Rohit Kumar Singh", email: "moderator.hns1@emqpgs.local", homeDept: hns.id },
    { name: "Dr. Ashwin Pathak", email: "contributor1.hns@emqpgs.local", homeDept: hns.id },
  ];
  ...
  for (const f of FACULTY) {
    await prisma.user.upsert({
      where: { email: f.email },
      update: {},
      create: { name: f.name, email: f.email, passwordHash: pwh, status: UserStatus.ACTIVE, homeDepartmentId: f.homeDept as string },
    });
  }
  ...
  // COE (institution-wide)
  await assignResponsibility("coe@emqpgs.local", ResponsibilityType.COE, ScopeType.INSTITUTION);

  // DEAN (institution-wide)
  await assignResponsibility("dean@emqpgs.local", ResponsibilityType.DEAN, ScopeType.INSTITUTION);

  // COORDINATORs (department-scoped)
  await assignResponsibility("coordinator.comp@emqpgs.local", ResponsibilityType.COORDINATOR, ScopeType.DEPARTMENT, comp.id);
  await assignResponsibility("coordinator.extc@emqpgs.local", ResponsibilityType.COORDINATOR, ScopeType.DEPARTMENT, extc.id);
  await assignResponsibility("coordinator.hns@emqpgs.local", ResponsibilityType.COORDINATOR, ScopeType.DEPARTMENT, hns.id);

  // MODERATORs will be assigned to question banks by coordinators at runtime (not seeded directly)
  ...
  // ──────────────────────────────────────────────
  const coeUser = await prisma.user.findFirst({
    where: { responsibilities: { some: { responsibility: ResponsibilityType.COE } } },
  });
  ...
  // ──────────────────────────────────────────────
  console.log("\n═══════════════════════════════════════════");
  console.log("SEED COMPLETE — RESPONSIBILITY-BASED AUTH");
  console.log("═══════════════════════════════════════════");
  ...
  console.log(`   Responsibility Assignments:`);
  console.log(`     • COE (Institution): coe@emqpgs.local`);
  console.log(`     • DEAN (Institution): dean@emqpgs.local`);
  console.log(`     • Coordinator (Department): 3 coordinators assigned`);
